...
- **`uvx` silently resolves to a stale semble version.** Without `--local-semble`, the with-semble arm runs `uvx --from "semble[mcp]" semble`. `uv`/`uvx` discover config (including `exclude-newer`) by walking up parent directories from the subprocess's cwd until they find a `pyproject.toml`. Cloned task repos under `repos/` have no `pyproject.toml` of their own (just `setup.py`/`setup.cfg`), so discovery walks all the way up to *this* project's `pyproject.toml` and picks up its own `[tool.uv] exclude-newer = "1 week"` — meaning every with-semble run silently tests whatever semble release was newest **more than a week before the run**, not the actual PyPI latest and definitely not this branch. Always use `--local-semble` to test current branch code; only treat PyPI-version results as meaningful if you've confirmed what `uvx --from "semble[mcp]" python -c "import semble; print(semble.version.__version__)"` actually resolves to from inside a cloned repo dir.
- **The without-semble arm is version-independent.** It never invokes the semble CLI/MCP server at all (config is stripped/disabled), so it doesn't need to be re-run when only the semble version under test changes — only the with-semble arm does. Re-running just `--with-semble-only` merges cleanly into existing results, keyed by `(backend, model, variant)`.
- **Bypass detection false-positives on path/word collisions.** `is_semble_shell_invocation` (in `utils.py`) flags a Bash command as a semble-CLI bypass only when `semble` appears as a standalone token. An earlier naive `"semble" in command` substring check produced false positives for any command referencing this project's own absolute path (`.../oss/semble/...`) or unrelated words containing the substring (e.g. scikit-learn's `ensemble` module) — both with- and without-semble runs were affected. If `[SEMBLE_BYPASS]` shows up unexpectedly, check whether it's a genuine CLI invocation before treating the run as contaminated.
- **Claude backend kill-safety.** `ClaudeBackend` temporarily empties the global `~/.agents/AGENTS.md` (symlinked from `~/.claude/CLAUDE.md`) during without-semble runs, since that file independently instructs falling back to `uvx --from "semble[mcp]" semble` and would otherwise leak semble usage into the baseline. SIGTERM/SIGINT (e.g. `TaskStop`) trigger an immediate restore; a hard kill (SIGKILL) leaves a `.benchmark_backup` file that self-heals (with a printed warning) the next time any script imports `backends/claude.py`. Don't run claude without-semble concurrently with another process that reads the same shared file.
