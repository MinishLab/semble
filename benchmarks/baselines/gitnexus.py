...
# benchmark repo names are already ASCII slugs; --name only rejects a few chars.
...
def _alias(repo: str) -> str:
    return f"semble-bench-{_ALIAS_RE.sub('-', repo)}"


def _cleanup_index(benchmark_dir: Path, alias: str) -> None:
    subprocess.run([_GITNEXUS, "remove", alias, "--force"], capture_output=True, timeout=30)
    shutil.rmtree(benchmark_dir / ".gitnexus", ignore_errors=True)


def _build_index(benchmark_dir: Path, alias: str) -> tuple[bool, float]:
    """Index a repo (embeddings on, no git-root requirement); return (success, elapsed_ms)."""
    _cleanup_index(benchmark_dir, alias)
    ...
    try:
        ...
        print(f"  WARNING: gitnexus analyze timed out after {_INDEX_TIMEOUT}s", file=sys.stderr)
        return False, (time.perf_counter() - started) * 1000
    elapsed_ms = (time.perf_counter() - started) * 1000
    if proc.returncode != 0:
        print(f"  WARNING: gitnexus analyze failed: {proc.stderr.strip()[:300]}", file=sys.stderr)
        return False, elapsed_ms
    return True, elapsed_ms


def _run_search(query: str, benchmark_dir: Path, alias: str, *, top_k: int) -> list[str]:
    """Return absolute file paths from gitnexus query's ranked `definitions` list."""
    cmd = [_GITNEXUS, "query", query, "--repo", alias, "--limit", str(top_k)]
    try:
    ...
    if proc.returncode != 0:
        return []
    # gitnexus logs status lines (e.g. "Loading embedding model...") before the JSON object.
    ...
    *,
    ...
    for task in tasks:
        ...
        for _ in range(_LATENCY_RUNS):
            started = time.perf_counter()
            file_paths = _run_search(task.query, benchmark_dir, alias, top_k=_TOP_K)
...
def _run_repo(spec: RepoSpec, tasks: list[Task], *, verbose: bool) -> RepoResult | None:
    ...
    ok, index_ms = _build_index(benchmark_dir, alias)
    if not ok:
        print(f"  SKIP: {spec.name} — gitnexus indexing failed", file=sys.stderr)
        _cleanup_index(benchmark_dir, alias)
        return None

    try:
        ndcg10, p50_ms = _evaluate_repo(tasks, benchmark_dir, alias, verbose=verbose)
    finally:
        _cleanup_index(benchmark_dir, alias)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark gitnexus on the semble benchmark suite.")
    add_filter_args(parser, verbose=True)
    return parser.parse_args()


def main() -> None:
    """Run the gitnexus baseline benchmark."""
    args = _parse_args()
    repo_specs, tasks = load_filtered_tasks(args.repo or None, args.language or None)

    print("gitnexus (knowledge graph: BM25 + local ONNX embeddings + RRF, ranked by `definitions`)", file=sys.stderr)
    ...
    summary = {
        "tool": "gitnexus",
        "note": "knowledge-graph search: SQLite/LadybugDB BM25 + local ONNX embeddings, ranked `definitions` list",
        "repos": [
            {
                "repo": r.repo,
                "language": r.language,
    ...
    if not args.repo and not args.language:
        out = save_results("gitnexus", summary)
