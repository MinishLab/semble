...
_ENV = {**os.environ, "CODEGRAPH_TELEMETRY": "0"}
...
def _cleanup_index(benchmark_dir: Path) -> None:
    shutil.rmtree(benchmark_dir / ".codegraph", ignore_errors=True)


def _build_index(benchmark_dir: Path) -> tuple[bool, float]:
    """Build a codegraph FTS5 + graph index for a repo; return (success, elapsed_ms)."""
    _cleanup_index(benchmark_dir)
    started = time.perf_counter()
    try:
        proc = subprocess.run(
            [_CODEGRAPH, "init", str(benchmark_dir)],
            capture_output=True,
            text=True,
        ...
        print(f"  WARNING: codegraph init timed out after {_INDEX_TIMEOUT}s", file=sys.stderr)
        return False, (time.perf_counter() - started) * 1000
    elapsed_ms = (time.perf_counter() - started) * 1000
    if proc.returncode != 0:
        print(f"  WARNING: codegraph init failed: {proc.stderr.strip()[:300]}", file=sys.stderr)
        return False, elapsed_ms
    return True, elapsed_ms


def _run_search(query: str, benchmark_dir: Path, *, top_k: int) -> list[str]:
    """Return absolute file paths from codegraph's ranked (FTS5) JSON output."""
    cmd = [_CODEGRAPH, "query", query, "--path", str(benchmark_dir), "--json", "--limit", str(top_k * 2)]
    try:
        proc = subprocess.run(cmd, capture_output=True, text=True, timeout=_SEARCH_TIMEOUT, env=_ENV)
    except subprocess.TimeoutExpired:
        return []
    if proc.returncode != 0:
        return []
    ...
    seen: dict[str, None] = {}
    for item in data:
        rel = item.get("node", {}).get("filePath", "")
        if rel:
...
def _run_repo(spec: RepoSpec, tasks: list[Task], *, verbose: bool) -> RepoResult | None:
    ...
    if not ok:
        print(f"  SKIP: {spec.name} — codegraph indexing failed", file=sys.stderr)
        _cleanup_index(benchmark_dir)
        return None
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark codegraph on the semble benchmark suite.")
    add_filter_args(parser, verbose=True)
    return parser.parse_args()


def main() -> None:
    """Run the codegraph baseline benchmark."""
    args = _parse_args()
    repo_specs, tasks = load_filtered_tasks(args.repo or None, args.language or None)

    print("codegraph (SQLite FTS5 + graph traversal, ranked `query` results)", file=sys.stderr)
    ...
    summary = {
        "tool": "codegraph",
        "note": "SQLite FTS5 symbol search + graph traversal; no embeddings. Does not support Haskell, "
        "Bash, Elixir, or Zig (indexes 0 symbols for those repos, scoring 0 rather than failing to "
        "index outright).",
        "repos": [
            {
                "repo": r.repo,
    ...
    if not args.repo and not args.language:
        out = save_results("codegraph", summary)
