...
class _EmbeddingCache:
    """Two-level embedding cache: in-memory dict backed by optional disk storage.

    Lookup order: memory → disk → miss.  Writes go to both layers when a
    :class:`_CacheSpec` is provided; memory-only otherwise.
    """
    ...
    def __init__(
        ...
        :param memory: Shared in-memory dict (typically ``SembleIndex._embedding_cache``).
    ...
    def get(self, key: str) -> EmbeddingMatrix | None:
        """Return the embedding for *key*, or ``None`` on a full miss.
    ...
    def put(self, key: str, embedding: EmbeddingMatrix) -> None:
        ...
        :raises Exception: Re-raises any I/O error from the atomic disk write,
