...
git -c advice.detachedHead=false clone -q --depth 1 --branch 0.115.6 https://github.com/fastapi/fastapi "$seed/fastapi"
# Warm the model cache so the recording doesn't hit the network or print an HF Hub warning.
env -i HOME="$seed" PATH="$SEMBLE_BIN_DIR:/usr/bin:/bin" "$SEMBLE_BIN_DIR/semble" search x "$seed/fastapi" -k 1 >/dev/null

# Record the demo for both dark and light themes.
cp -R "$seed/." "$dark/"
cp -R "$seed/." "$light/"
